...
class PredictionPage:
    ...
    def get_position_form(self):
        with st.sidebar.form(key="Ativo"):
            ...
            carbon_intensity = st.number_input(
                "Estimativa da intensidade de carbono em tCO2e/Milhões de R$",
                min_value=0,
            )
    ...
    def get_companies_ticker() -> list:
        data = get_data()
        return (
            data.loc[data["fiscal_year"].eq(2019), "ticker"]
            .sort_values()
            .unique()
            .tolist()
        )
    ...
    def show_predictions(
        submit_button, company, cagr_revenue, cagr_carbon, carbon_intensity
    ):
        if submit_button:
            ...
            get_carbon_offset_time_series_forecast(col2, company, cagr_carbon / 100)
...
def app():
    ...
    prediction_page.show_predictions(
        submit_button, company, cagr_revenue, cagr_carbon, carbon_intensity
    )
